...
We're going to start with a very basic csv with 500 rows. It has two columns:
...
The first step is to load the data. We do this by creating a **variable** to store the data once its loaded. We also use a default function in R `read.csv()`.

The `read.csv()` function reads data stored in common separated value (`.csv`) files.

```{r load-ice-cream, exercise=TRUE}
ice_cream <- read.csv("./www/ice_cream_sales.csv")
```

For the line above to work, you'll need to change the `path`. The `path` is part of the code between the quotation marks `"`. It points to where the data is stored on your computer. 

If you're running this on a web browser, you won't need to change anything. If you're using this code on your computer, just change the part in the quotation marks to where the data is stored on your computer. 
